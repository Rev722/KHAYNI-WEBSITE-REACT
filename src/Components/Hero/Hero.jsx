import React from 'react'
import './Hero.css'
import dark_arrow from '../../assets/dark-arrow.png'

// Hero Section Elements

const Hero = () => {
  return (
    <div className='hero container'>
      <div className="hero-text">
        <h1> Who We Are </h1>
        <p> We are a dynamic Innovative IT and Training solutions company dedicated to providing
            cutting-edge solutions tailored to meet the diverse needs of modern digital, 21 st century
            businesses. Our mission is to bridge the gap between technology and business, delivering
            innovative solutions and expert training to enhance productivity, efficiency, and growth.
            Khayni is driven by a firm belief that growth brings about unlimited dreams and high-quality
            performance. We are an organization that produces the highest level of results that unleash
            full potential and unlock talents. </p>
      </div>
    </div>
  )
}

export default Hero
